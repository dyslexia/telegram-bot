...
async def pool_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bscurl = api.bnbprice + keys.bsc
    bscresponse = requests.get(bscurl)
    bscdata = bscresponse.json()
    bscvalue = float(bscdata["result"]["ethusd"])
    bscpoolurl = api.bnbbalance + items.lpreserveca + '&tag=latest' + keys.bsc
    ...
    arbpoolurl = api.ethbalancearb + items.lpreserveca + '&tag=latest' + keys.arb
    ...
    polyurl = api.maticprice + keys.poly
    polyresponse = requests.get(polyurl)
    polydata = polyresponse.json()
    polyvalue = float(polydata["result"]["maticusd"])
    polypoolurl = api.maticbalance + items.lpreserveca + '&tag=latest' + keys.poly
    ...
    optipoolurl = api.ethbalanceopti + items.lpreserveca + '&tag=latest' + keys.opti
    scraper = cloudscraper.create_scraper(delay=10, browser={'custom': 'ScraperBot/1.0', })
...
async def x7r_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if api.price["x7r"]["usd_24h_change"] is None:
        api.price["x7r"]["usd_24h_change"] = 0
    if dollar:
        amount = round(float(chain[1:]) / float(api.price["x7r"]["usd"]), 2)
        im1 = Image.open((random.choice(items.blackhole)))
        ...
        await update.message.reply_photo(
            ...
            parse_mode='Markdown')
    if chain.isdigit():
        amount = float(chain) * float(api.price["x7r"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7rlogo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(r'media\FreeMonoBold.ttf', 26)
    ...
    if chain == "" or chain == "eth":
        ...
        x7rtokendollar = float(api.price["x7r"]["usd"]) * float(x7rtoken) / 10 ** 18
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7rlogo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(R'media\FreeMonoBold.ttf', 28)
        ...
        i1.text((26, 30),
                f'X7R Info (ETH)\n\n'
                f'X7R Price: ${api.price["x7r"]["usd"]}\n'
                f'24 Hour Change: {round(api.price["x7r"]["usd_24h_change"], 1)}%\n'
                f'Market Cap:  ${"{:0,.0f}".format(api.price["x7r"]["usd"] * items.supply)}\n'
                f'24 Hour Volume: ${"{:0,.0f}".format(api.price["x7r"]["usd_24h_vol"])}\n'
...
async def x7dao_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chain = " ".join(context.args).lower()
    dollar = chain.startswith("$")
    if api.price["x7dao"]["usd_24h_change"] is None:
        api.price["x7dao"]["usd_24h_change"] = 0
    if dollar:
        amount = float(chain[1:]) / float(api.price["x7dao"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
    ...
    if chain == "proposal":
        chain = "500000"
    if chain == "500000":
        amount = float(chain) * float(api.price["x7dao"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7daologo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(r'media\FreeMonoBold.ttf', 26)
    ...
    if chain == "":
        ...
        x7daotokendollar = float(api.price["x7dao"]["usd"]) * float(x7daotoken) / 10 ** 18
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7daologo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(r'media\FreeMonoBold.ttf', 26)
        ...
        i1.text((28, 36),
                f'X7DAO Info (ETH)\n\n'
                f'X7DAO Price: ${api.price["x7dao"]["usd"]}\n'
                f'24 Hour Change: {round(api.price["x7dao"]["usd_24h_change"], 1)}%\n'
                f'Market Cap:  ${"{:0,.0f}".format(api.price["x7dao"]["usd"] * items.supply)}\n'
                f'24 Hour Volume: ${"{:0,.0f}".format(api.price["x7dao"]["usd_24h_vol"])}\n'
...
async def x7101_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if api.price["x7101"]["usd_24h_change"] is None:
        api.price["x7101"]["usd_24h_change"] = 0
    if dollar:
        amount = float(chain[1:]) / float(api.price["x7101"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        ...
        await update.message.reply_photo(
            ...
            caption=f'{chain} Is currently worth:\n\n{"{:0,.0f}".format(amount)}' 
                    f' X7101 (ETH) Tokens (before tax)\n\n{quote}', parse_mode='Markdown')
    if chain.isdigit():
        amount = float(chain)*float(api.price["x7101"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7101logo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(r'media\FreeMonoBold.ttf', 26)
    ...
    if chain == "" or chain == "eth":
        ...
        i1.text((28, 36),
                f'X7101 Info (ETH)\n\n'
                f'X7101 Price: ${api.price["x7101"]["usd"]}\n'
                f'24 Hour Change: {round(api.price["x7101"]["usd_24h_change"]),1}%\n'
                f'Market Cap:  ${"{:0,.0f}".format(api.price["x7101"]["usd"] * items.supply)}\n'
                f'24 Hour Volume: ${"{:0,.0f}".format(api.price["x7101"]["usd_24h_vol"])}\n'
        ...
        await update.message.reply_photo(
            ...
            f'24 Hour Change: {round(api.price["x7101"]["usd_24h_change"],1)}%\n'
            f'Market Cap:  ${"{:0,.0f}".format(api.price["x7101"]["usd"]*items.supply)}\n'
            f'24 Hour Volume: ${round(api.price["x7101"]["usd_24h_vol"])}\n'
            f'Holders: {x7101holders}\n\n'
            f'*X7101 Contract*\n`{items.x7101ca}`\n\n{quote}',
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([
...
async def x7102_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if api.price["x7102"]["usd_24h_change"] is None:
        api.price["x7102"]["usd_24h_change"] = 0
    if dollar:
        amount = float(chain[1:]) / float(api.price["x7102"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        ...
        await update.message.reply_photo(
            ...
            parse_mode='Markdown')
    if chain.isdigit():
        amount = float(chain) * float(api.price["x7102"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
        im2 = Image.open(items.x7102logo)
        im1.paste(im2, (720, 20), im2)
        myfont = ImageFont.truetype(r'media\FreeMonoBold.ttf', 26)
    ...
    if chain == "":
        ...
        i1.text((28, 36),
                f'X7102 Info (ETH)\n\n'
                f'X7102 Price: ${api.price["x7102"]["usd"]}\n'
                f'24 Hour Change: {round(api.price["x7102"]["usd_24h_change"],1)}%\n'
                f'Market Cap:  ${"{:0,.0f}".format(api.price["x7102"]["usd"] * items.supply)}\n'
                f'24 Hour Volume: ${"{:0,.0f}".format(api.price["x7102"]["usd_24h_vol"])}\n'
...
async def x7103_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dollar:
        amount = float(chain[1:]) / float(api.cg["x7103"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
...
async def x7104_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dollar:
        amount = float(chain[1:]) / float(api.cg["x7104"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
...
async def x7105_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dollar:
        amount = float(chain[1:]) / float(api.cg["x7105"]["usd"])
        im1 = Image.open((random.choice(items.blackhole)))
...
async def mcap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    x7102cap = (api.cg["x7102"]["usd"]) * items.supply
    x7103cap = (api.cg["x7103"]["usd"]) * items.supply
    x7104cap = (api.cg["x7104"]["usd"]) * items.supply
    x7105cap = (api.cg["x7105"]["usd"]) * items.supply
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cg = CoinGeckoAPI()
    tokenprice = cg.get_price(ids=tokenid, vs_currencies='usd', include_24hr_change='true', include_24hr_vol='true')
    if search == "":
        im1 = Image.open((random.choice(items.blackhole)))
...
async def treasury_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    comx7rurl = api.tokenbalanceeth + items.x7rca + '&address=' + items.commultieth + '&tag=latest' + keys.ether
    ...
    comx7durl = api.tokenbalanceeth + items.x7dca + '&address=' + items.commultieth + '&tag=latest' + keys.ether
...
async def burn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chain == "polygon" or chain == "poly":
        burnurl = api.tokenbalanceapipoly + items.x7rca + '&address=' + items.dead + '&tag=latest' + keys.poly
        burnresponse = requests.get(burnurl)
        burndata = burnresponse.json()
        burndata["result"] = int(burndata["result"][:-18])
        result = round(((burndata["result"] / items.supply) * 100), 6)
    ...
    if chain == "arbitrum" or chain == "arb":
        burnurl = api.tokenbalancebsc + items.x7rca + '&address=' + items.dead + '&tag=latest' + keys.arb
        burnresponse = requests.get(burnurl)
        burndata = burnresponse.json()
        burndata["result"] = int(burndata["result"][:-18])
        result = round(((burndata["result"] / items.supply) * 100), 6)
    ...
    if chain == "optimism" or chain == "arb":
        burnurl = api.tokenbalancebsc + items.x7rca + '&address=' + items.dead + '&tag=latest' + keys.opti
        burnresponse = requests.get(burnurl)
        burndata = burnresponse.json()
        burndata["result"] = int(burndata["result"][:-18])
        result = round(((burndata["result"] / items.supply) * 100), 6)
