...
async def nft(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_photo(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Mint Here", url="https://x7.finance/x/nft/mint"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="Ecosystem Maxi", url=f"{chain_url}{ca.eco}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="Liquidity Maxi", url=f"{chain_url}{ca.liq}"
                        )
                    ],
                    [InlineKeyboardButton(text="DEX Maxi", url=f"{chain_url}{ca.dex}")],
                    [
                        InlineKeyboardButton(
                            text="Borrowing Maxi", url=f"{chain_url}{ca.borrow}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(
                            text="Pioneer", url=f"{chain_url}{ca.pioneer}"
                        )
                    ],
                ]
            ),
    ...
    if chain == "opti" or chain == "optimism" or chain == "op":
        ...
        magister_count = int(
            api.get_nft_holder_count(ca.magister, "?chain=optimism-main")
        )
    ...
    if chain == "arb" or chain == "arbitrum":
        ...
        magister_count = int(
            api.get_nft_holder_count(ca.magister, "?chain=arbitrum-main")
        )
...
async def question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"Thanks {update.effective_message.from_user.username}, "
...
async def treasury(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chain == "" or chain == "eth":
        ...
        pioneer_dollar = (
            float(pioneer_eth) * float(api.get_native_price("eth")) / 1**18
        )
                ...
                [
                    ...
                    [
                        InlineKeyboardButton(
                            ...
                            url=f"{url.ether_address}{ca.dev_multi_eth}",
                        )
                    ],
                    [
                        InlineKeyboardButton(
